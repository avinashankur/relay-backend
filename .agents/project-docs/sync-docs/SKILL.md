...
> "You said 'process the document' — do you mean _load_ it (produce a
> `Document`), _chunk_ it (produce `Chunk[]`), or _ingest_ it (the full
...
Keep it structural, not chatty. The architecture doc describes _what_ exists and
_why_ it's structured that way — not the conversation that led to the decision
