...
- [ ] `DOCS-06` `P2` `Done` Add Docker multi-stage builds and docker-compose setup for containerized local development and deployment. Design doc written at `docs/docker.md`; actual `Dockerfile`, `Dockerfile.worker`, and `docker-compose.yml` in `infra/docker/` not yet created.
